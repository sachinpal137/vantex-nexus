...
export default function SettingsHub() {
  ...
  const handleExport = async () => {
    // FIX: Removed .env dependency for GET export. Using simple confirmation.
    const confirmExport = window.confirm("Do you want to download a backup of the public cloud data?");
    ...
    try {
      // FIX: Fetching without auth headers since GET is now public
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center bg-slate-900/50 p-6 rounded-2xl border border-slate-800/60 shadow-xl backdrop-blur-sm">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2"><span>⚙️</span> System Settings & Data</h2>
        </div>
      </div>
     
      {status && <div className="mb-6 p-4 bg-slate-900 border-l-4 border-emerald-500 text-emerald-400 font-medium rounded-xl shadow-lg">{status}</div>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-900/40 border border-slate-800/60 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          <div>
            ...
            <p className="text-slate-400 text-sm mb-6">Download a complete JSON snapshot of all active deals, clients, invoices, and expenses.</p>
          </div>
          <button onClick={handleExport} disabled={isExporting} className="px-4 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50">
        ...
        <div className="bg-slate-900/40 border border-rose-950/40 p-6 rounded-2xl shadow-xl flex flex-col justify-between">
          ...
          <button onClick={handleClearDataWarning} className="px-4 py-3 bg-rose-600/10 hover:bg-rose-600 text-rose-500 border border-rose-600/30 border-dashed font-semibold rounded-xl transition-all w-full flex items-center justify-center cursor-pointer">
